...
%%获取小于Lv级、Day天未登录、且非VIP的玩家
get_to_delete_players(Lv, Day) ->
	TimeLimit = util:unixtime() - Day*24*3600,
	?DB_MODULE:select_all(player, "id",[{level, "<", Lv}, {last_login_time, "<", TimeLimit}, {vip_gold, "<", 1}],[],[]).

%%按角色ID删除表中的数据
delete_table_data_by_uid(TableName, Key) ->
	?DB_MODULE:delete(TableName, Key).
delete_player(Uid) ->
	?DB_MODULE:delete(player, [{id,Uid}]).
	
%%删除player、user、infant_ctrl_byuser表中的玩家数据	
